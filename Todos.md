...
The number of full-team budget days remaining is currently being compared to the number of calendar days until the end date in both the hero card and the modal. This is inaccurate as the team does not work on weekends. In the same way that the burndown chart excludes wekends, we should make the comparison of full-team budget days remaining (days the full team can be paid to work) against actual weekdays which is when they are working. You can note the calendar days afterwards but the budget days direct comparison should be apples-and-apples.
